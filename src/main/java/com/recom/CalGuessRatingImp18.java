 ...
 */
public class CalGuessRatingImp18 extends CalGuessRating {
	 @Override
	    protected void calAvgValue(List<Rating> list) {
		   CalUserVector18.cal(list);
		   System.out.println("CalGuessRatingImp18");
	    }

	    @Override
	    protected void calUserSimilarity(int i, int j, List<Rating> list)  {
	        Set<Integer> itemSet = CalUserVector18.calIncludeSet(i, j);
	        double sum1 = 0, sum2 = 0, sum3 = 0,wij = 100.0;
	        // wij指的是gaptuv
	        if (itemSet.size() != 0) {
	            wij = CalUserVector18.calGap(i, j, itemSet);
            }
	        ...
	        map.put(j, ( my * sum1 / (Math.sqrt(sum2) * Math.sqrt(sum3))));
	        similarMatrix.put(i, map);
	        Map<Integer, Double> map1 = similarMatrix.get(j);
	        if (map1 == null) {
	            map1 = new HashMap<>();
	        }
	        map1.put(i, (my * sum1 / (Math.sqrt(sum2) * Math.sqrt(sum3))));
