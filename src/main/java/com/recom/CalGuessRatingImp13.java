 ...
 */
public class CalGuessRatingImp13 extends CalGuessRating {
	 @Override
	    protected void calAvgValue(List<Rating> list) {
		   CalUserVector13.cal(list);
		   System.out.println("CalGuessRatingImp13");
	    }

	    @Override
	    protected void calUserSimilarity(int i, int j, List<Rating> list)  {
	        Set<Integer> itemSet = CalUserVector13.calIncludeSet(i, j);
	        double sum1 = 0, sum2 = 0, sum3 = 0,wij = 0;
	        for (Integer itemId : itemSet) {
				// wij指的是gaptuv
				wij = CalUserVector13.calGap(i, j, itemSet);
