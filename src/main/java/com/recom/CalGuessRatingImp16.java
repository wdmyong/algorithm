 ...
 */
@Deprecated
public class CalGuessRatingImp16 extends CalGuessRating {
	 @Override
	    protected void calAvgValue(List<Rating> list) {
		   CalUserVector16.cal(list);
		   System.out.println("CalGuessRatingImp12");
	    }

	    @Override
	    protected void calUserSimilarity(int i, int j, List<Rating> list)  {
	        Set<Integer> itemSet = CalUserVector16.calIncludeSet(i, j);
	        double sum1 = 0, sum2 = 0, sum3 = 0,wij = 0;
	        for (Integer itemId : itemSet) {
				// wij指的是gaptuv
				wij = CalUserVector16.calGap(i, j, itemSet);
