...
/**
 * Created by juanchen on 2017/3/11.
 * 实现子类           4.2(新版)
 * 根据什么条件计算相似性，重写calAvgValue
 * 计算相似性的具体方法，重写calUserSimilarity
 */
public class CalGuessRatingImp11 extends CalGuessRating {
	 @Override
	    protected void calAvgValue(List<Rating> list) {
		   System.out.println("CalGuessRatingImp11");
		   CalUserVector.cal(list);
	    }

	    @Override
	    protected void calUserSimilarity(int i, int j, List<Rating> list)  {
	        Set<Integer> itemSet = CalUserVector.calIncludeSet(i, j);
	        double sum1 = 0, sum2 = 0, sum3 = 0,wij = 0;
	        for (Integer itemId : itemSet) {
				// wij指的是gaptuv
				wij = CalUserVector.calGap(i, j, itemSet);
