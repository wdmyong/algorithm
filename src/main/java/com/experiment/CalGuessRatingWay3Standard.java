 ...
 */
public class CalGuessRatingWay3Standard extends CalGuessRating {
	 @Override
	    protected void calAvgValue(List<Rating> list) {
		   CalUserVectorWay3Standard.cal(list);
		   System.out.println("CalGuessRatingImp19");
	    }

	    @Override
	    protected void calUserSimilarity(int i, int j, List<Rating> list)  {
	        Set<Integer> itemSet = CalUserVectorWay3Standard.calIncludeSet(i, j);
	        
	        double sum1 = 0, sum2 = 0, sum3 = 0,wij = 1.0;
	        // wij指的是gaptuv
	        if (itemSet.size() != 0) {
	            wij = CalUserVectorWay3Standard.calGap(i, j, itemSet);
            }
