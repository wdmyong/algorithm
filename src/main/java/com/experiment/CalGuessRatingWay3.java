 ...
 */
public class CalGuessRatingWay3 extends CalGuessRating {
	 @Override
	    protected void calAvgValue(List<Rating> list) {
		   CalUserVectorWay3.cal(list);
		   System.out.println("CalGuessRatingImp20");
	    }

	    @Override
	    protected void calUserSimilarity(int i, int j, List<Rating> list)  {
	        Set<Integer> itemSet = CalUserVectorWay3.calIncludeSet(i, j);
	        double sum1 = 0, sum2 = 0, sum3 = 0,wij = 1.0;
	        // wij指的是gaptuv
	        if (itemSet.size() != 0) {
	            wij = CalUserVectorWay3.calGap(i, j, itemSet);
            }
	        ...
	        double res = 0;
	        if (itemSet.size() != 0) {
	            System.out.println("trkyfuuijkhgjgjkgkjgjgjg\t" + my + "\t" + wij);
