 ...
 */
public class CalGuessRatingWay3Modify extends CalGuessRating {
    ...
    protected void calAvgValue(List<Rating> list) {
        ...
        CalUserVectorWay3Modify.cal(list);
    }

    @Override
    protected void calUserSimilarity(int i, int j, List<Rating> list)  {
        Set<Integer> itemSet = CalUserVectorWay3Modify.calIncludeSet(i, j);
        double sum1 = 0, sum2 = 0, sum3 = 0, wij = 1.0;
        if (itemSet.size() != 0) {
            wij = CalUserVectorWay3Modify.calGap(i, j, itemSet);
        }
        for (Integer itemId : itemSet) {
            sum1 += (scoreMatrix.get(i).get(itemId)) *
                    (scoreMatrix.get(j).get(itemId)) *
                    (ratingMatrix.get(i).get(itemId) - avgMap.get(i)) *
                    (ratingMatrix.get(j).get(itemId) - avgMap.get(j));
            sum2 += scoreMatrix.get(i).get(itemId) * (ratingMatrix.get(i).get(itemId) - avgMap.get(i)) *
                    (ratingMatrix.get(i).get(itemId) - avgMap.get(i));
            sum3 += scoreMatrix.get(j).get(itemId) * (ratingMatrix.get(j).get(itemId) - avgMap.get(j)) *
                    (ratingMatrix.get(j).get(itemId) - avgMap.get(j));
        }
        ...
        double result = 0;
        double my = (0.25*Math.pow(Math.E, -wij) + 0.75);
        if (itemSet.size() > 0) {
            result = my * sum1 / (Math.sqrt(sum2) * Math.sqrt(sum3));
        }
        map.put(j, result);
